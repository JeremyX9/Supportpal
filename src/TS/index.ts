...
class ExtensionBuilder {
    ...
    public async build(): Promise<void> {
        ...
        this.setLink(managerNode, await this.dataProvider.getManager().then((manager) => { return manager.title; }), await this.dataProvider.getManager().then((manager) => { return manager.login; }));
        ...
        this.setLink(mailAdminNode, await this.dataProvider.getMailSettingsAdmin().then((mailSettingsAdmin) => { return mailSettingsAdmin; }), await this.dataProvider.getMailSettingsAdmin().then((mailSettingsAdmin) => { return mailSettingsAdmin; }));
        this.setContent(imapServerNode,"IP: " + await this.dataProvider.getImap().then((imap) => { return imap[0]; }));
        this.setContent(imapPortNode, "Port: " + await this.dataProvider.getImap().then((imap) => { return imap[1]; }));
        this.setContent(popServerNode, "IP: " + await this.dataProvider.getPop().then((pop) => { return pop[0]; }));
        this.setContent(popPortNode, "Port: " + await this.dataProvider.getPop().then((pop) => { return pop[1]; }));
        this.setContent(smtpServerNode, "IP: " + await this.dataProvider.getSmtp().then((smtp) => { return smtp[0]; }));
        this.setContent(smtpPortNode, "Port: " + await this.dataProvider.getSmtp().then((smtp) => { return smtp[1]; }));
